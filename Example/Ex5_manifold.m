...
plot3(x_corrected(:,1), x_corrected(:,2),x_corrected(:,3),'k');
...
hold off
f1_name = strcat('Ex5_manifold_mu=',num2str(mu),'_xn=',num2str(x_n(1)),'_zn=',num2str(x_n(3)),'_ydotn=',num2str(x_n(5)),'_tn=',num2str(t_n),'_xpert=',num2str(xpert),'_tf=',num2str(tf));
f1_name = strrep(f1_name,'.',',');
save_fig(f1,f1_name,view_angle);
if Lyapunov == false
  legend('off');
  f1_name_xy = strcat(f1_name,'_xy');
  save_fig(f1,f1_name_xy,[0 90]);
  f1_name_xz = strcat(f1_name,'_xz');
  save_fig(f1,f1_name_xz,[0 0]);
  f1_name_yz = strcat(f1_name,'_yz');
  save_fig(f1,f1_name_yz,[90 0]);
end
