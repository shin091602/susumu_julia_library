%% Pseudo arclength continuation for southern halo family around L2 in Sun-Earth CR3BP
%% By:Soi Yamaguchi
% copied over from the parts of MATLAB code written by Kanta Ikeda
...
x0 = [1.01041210799432;0;-0.00701710214085124;0;-0.0132251420919099;0];
T = 3.03765488729945;
...
%% SAVE MAT --ORBIT DATA - Halo
cd mat/
save("inidata_torus_southern_halo.mat","orbit_data");
cd ..
...
axis square
xlabel('$Period$[-]');
ylabel('$Jacobi\quad Constant$[-]');
% hodograph
plot(orbit_data(1:p("count_max")/p("plot_interval"),end),orbit_data(1:p("count_max")/p("plot_interval"),end-1),'k','LineWidth',1.5);
hold off
...
for i=1:length(orbit_data)
    ...
    % stability index
    for j=1:6
      if (abs(D(j,j)-1)<1e-3) && (imag(D(j,j))~=0)
...
%% SAVE RESULTS
cd fig/family/
h_name = strcat('southern_halofamily_SE_mu=', num2str(p("mu")), '_x0=', num2str(x0_corrected1(1,1)), '_z0=', num2str(x0_corrected1(1,3)),...
