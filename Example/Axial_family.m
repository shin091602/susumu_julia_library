%% Pseudo arclength continuation for axial family around L2 in Sun-Earth CR3BP
%% By:Soi Yamaguchi
...
x0 = [1.01095593036708;0;0;0;-0.0198535102007301;0.0190210211349253];
T = 4.21697099424615;
...
p("count_max")     = 2000;
p("plot_interval") = 100;
p("Jacobi_min") = 3.000091643722067;
p("Jacobi_max") = 3.000137845402229;
...
for iteration = 1:p("iteration_limit")
  [x02_ast, t02_ast, C, G] = fun_differential_correction_axial_PAC(x_n_L2, t_n_L2, x02, t02, p("scale"), delta, p("mu"), options_ODE);
...
cd mat/
save("inidata_torus_axial.mat","orbit_data");
cd ..

%% SAVE RESULTS
cd fig/family/
haxf = strcat('Axial_family_SE_mu=', num2str(p("mu")), '_x0=', num2str(x0_corrected(1,1)), '_y0dot=', num2str(x0_corrected(1,5)),'_zdot0=', num2str(x0_corrected(1,6)), '_t0=', num2str(t0_corrected(1)), '_scale=', num2str(p("scale")),'_count_max=', num2str(p("count_max")), '_plot_interval=', num2str(p("plot_interval")),'C_min=',num2str(p("Jacobi_min")),'C_max=',num2str(p("Jacobi_max")));
haxf = strrep(haxf,'.',',');
save_fig(hax,haxf,[-39 31]);

hStf = strcat('Axial_stability_SE_mu=', num2str(p("mu")), '_x0=', num2str(x0_corrected(1,1)), '_y0dot=', num2str(x0_corrected(1,5)),'_zdot0=', num2str(x0_corrected(1,6)), '_t0=', num2str(t0_corrected(1)), '_scale=', num2str(p("scale")),'_count_max=', num2str(p("count_max")), '_plot_interval=', num2str(p("plot_interval")),'C_min=',num2str(p("Jacobi_min")),'C_max=',num2str(p("Jacobi_max")));
hStf = strrep(hStf,'.',',');
save_fig(hSt,hStf,[-39 31]);
cd ..
cd ..
